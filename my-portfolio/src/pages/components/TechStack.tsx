...
import {
  ...
  SiDjango, SiFlutter, SiStyledcomponents, SiPostman
} from 'react-icons/si'
    ...
    <section className="py-12 px-6 max-w-6xl mx-auto text-center">
      ...
      <div className="grid gap-10 md:grid-cols-2">
        {categories.map((group, index) => (
          <div key={index}>
            <h3 className="text-lg font-semibold text-pink-500 mb-4">{group.title}</h3>
            <div className="flex flex-wrap justify-center gap-6 text-slate-400">
              {group.items.map(({ name, icon: Icon }, i) => (
                <div key={i} className="flex flex-col items-center w-20 hover:text-pink-500 transition-colors">
                  <Icon size={32} />
